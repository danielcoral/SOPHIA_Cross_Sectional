...
clusdiseasesumfx <- function(diseasedf, clusdat){
    ...
    joindf <- dplyr::inner_join(probs, dxdf, by = "eid")
...
incidencesumfx <- function(incidencedf, X){
    ...
    mediqrtime <- quantile(incdf$outcome_timeyrs, probs = c(.025, .25, .5, .75, .975), na.rm = TRUE, names = FALSE)
    data.frame(
        ntotal = ntotal,
        ncases = ncases, 
        personyrs = personyrs,
        ...
        avgtime = avgtime,
        sdtime = sdtime,
        medtime = mediqrtime[3],
        timeq2.5 = mediqrtime[1],
        timeq25 = mediqrtime[2],
        timeq75 = mediqrtime[4],
        timeq97.5 = mediqrtime[5]
    )
}
